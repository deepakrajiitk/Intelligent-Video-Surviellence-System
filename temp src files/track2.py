...
src_points = np.float32([(545, 230), (488, 886), (1134, 213), (1258, 890)])
...
def run(
    ...
    video_id = 0 if webcam else source[:-4]
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            imc = im0.copy() if (save_crop or save_db) else im0  # for save_crop
            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    for j, (output) in enumerate(outputs[i]):
                        ...
                        nth_center = (0, 0) if len(output[7])<20 else output[7][-20][1];
                        if(id==1):
                            print(frame_idx)
                        ...
                        if save_vid or save_crop or show_vid or save_db:  # Add bbox to image
                            ...
                            label = None if hide_labels else (f'{id} {names[c]} {speed} {direction}' if hide_conf else \
